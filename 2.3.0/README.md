...
GDAL2:2.3.0 Docker images (with Python 3).

```bash
docker run --rm -it geographica/gdal2:2.3.0
```

Using a data volume
```bash
$ export DATAFOLDER="-v /folder_with_your_testdata/:/home/datafolder"
$ docker run $DATAFOLDER --name gdal2 -it --rm geographica/gdal2:2.3.0 /bin/bash
```
...
$ cd docker_gdal2
$ docker build -t geographica/gdal2:2.3.0 .
```
